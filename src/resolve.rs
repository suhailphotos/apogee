...
impl<'a> Resolver<'a> {
  ...
  pub fn resolve(&self, input: &str) -> Result<String> {
    ...
    while i < bytes.len() {
      if bytes[i] == b'{' {
        ...
        let repl = self.token_value(token)
  ...
  fn token_value(&self, token: &str) -> Option<String> {
    ...
    match token {
      ...
      "shell" => Some(self.ctx.shell_type.map(|s| s.to_string()).unwrap_or_else(|| "unknown".to_string())),

      // Prefer runtime env for xdg so .env can override, but fall back to ctx
      "xdg_config_home" => self.env.get("XDG_CONFIG_HOME").cloned().or_else(|| Some(self.ctx.xdg_config_home.to_string_lossy().to_string())),
      ...
      "userprofile" => self.env.get("USERPROFILE").cloned().or_else(|| self.env.get("HOME").cloned()),
      "username" => self.env.get("USERNAME").cloned().or_else(|| self.env.get("USER").cloned()),
